...
def generate_pdf(bill_data):
    ...
    for data in bill_data:
        ...
        c.drawString(50, y-80, f"Total Price: {data[4]}")
        y -= 100
...
def generate_bill():
    ...
    cred = credentials.Certificate(r"C:\Users\tannu\OneDrive\Desktop\restro\assets\robot-f8bed-firebase-adminsdk-b95ir-7e3e89b004.json")
    ...
    if delivered_orders_ref:
        for key, order in delivered_orders_ref.items():
            ...
            if food_id is not None:
                ...
                total_price = order.get("total_price", 0)  # Use 0 as default if total_price is not found

                # Add data to the bill data list
                bill_data.append((food_id, table_number, datetime_value, "\n".join(selected_items), total_price))
            else:
...
def save_bill_to_file(bill_data):
    with open("bill.txt", "w") as file:
        ...
        for data in bill_data:
            ...
            file.write("Items:\n")
            file.write(f"{data[3]}\n")
            file.write(f"Total Price: {data[4]}\n")
            file.write("-----------------\n")
...
def show_bill_details(bill_data):
    ...
    for data in bill_data:
        ...
        bill_content.insert(tk.END, f"Total Price: {data[4]}\n")
...
def generate_invoice_text(data):
    ...
    invoice_text = f"SHASWAT RESTAURANT\n"
    invoice_text += "-----------------\n"
    ...
    invoice_text += "-----------------\n"
    invoice_text += f"Total Price: {data[4]}\n"
...
if __name__ == "__main__":
    ...
    # Create a table
    tree = ttk.Treeview(root, columns=("Order ID", "Table Number", "Date & Time", "Items", "Total Price"), show="headings")
