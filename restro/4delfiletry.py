    ...
    try:
        ...
        if orders_ref is not None:
            for order_key, order_value in orders_ref.items():
                ...
                total_price = order_value.get('total_price', '')  # Assuming total_price is stored in Firebase
                tag = 'even' if food_id_counter % 2 == 0 else 'odd'
                pending_tree.insert('', tk.END, values=(order_key, food_id, table_number, datetime_value, ", ".join(selected_items), total_price), tags=(tag,))
                food_id_counter += 1
        ...
        if delivered_orders_ref is not None:
            ...
            for order_key, order_value in delivered_orders_ref.items():
                ...
                tag = 'even' if food_id_counter % 2 == 0 else 'odd'
                delivered_tree.insert('', tk.END, values=(order_key, food_id, table_number, datetime_value, ", ".join(selected_items), total_price), tags=(tag,))
                food_id_counter += 1
...
def move_to_delivered(event):
    try:
        ...
        data = {"order_id": values[1], "table_number": values[2], "selected_items": values[4].split(", "), "datetime": current_datetime, "total_price": values[5]}
...
image = Image.open(r"C:\Users\tannu\OneDrive\Desktop\restro\assets\logo.png")
...
pending_tree = ttk.Treeview(pending_frame, columns=('Unique ID', 'Order ID', 'Table Number', 'Date', 'Food Item', 'Total Price'), show='headings')
...
pending_tree.heading('Total Price', text='Total Price')  # New heading for Total Price
...
pending_tree.column('Total Price', width=100)  # Adjust width as needed
...
delivered_tree = ttk.Treeview(delivered_frame, columns=('Unique ID', 'Order ID', 'Table Number', 'Date', 'Food Item', 'Total Price'), show='headings')
...
delivered_tree.heading('Total Price', text='Total Price')  # New heading for Total Price
...
delivered_tree.column('Total Price', width=100)  # Adjust width as needed
