...
["KADHAI PANEER- 285/-",
    ...
    def confirm_order(self):
        ...
        current_datetime = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        data = {"order_id": order_id, "table_number": table_number, "datetime": current_datetime, "selected_items": selected_items_dict}
        self.db.child("orders").push(data)
        ...
        tts = gTTS(text=confirmation_message, lang='en')
        tts.save("confirmation.mp3")
        os.system("mpg321 confirmation.mp3")
        os.remove("confirmation.mp3")
        ...
        subprocess.run(["python", "/home/ras2024/Downloads/shaswat/shaswat/lok.py"])
        #self.destroy()

def start_second_file():
    # Call the second file using subprocess

    subprocess.run(["python", "/home/ras2024/Downloads/shaswat/shaswat/lok.py", "--app", str(app)])
